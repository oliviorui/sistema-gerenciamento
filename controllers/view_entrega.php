...
if (!$row) { echo "<p>Entrega não encontrada.</p>"; exit(); }

// Regras de acesso (igual download)
if ($tipo === 'docente') {
  if ((int)$row['id_docente'] !== $idUser) { http_response_code(403); echo "<p>Acesso negado.</p>"; exit(); }
} elseif ($tipo === 'estudante') {
  if ((int)$row['id_estudante'] !== $idUser) { http_response_code(403); echo "<p>Acesso negado.</p>"; exit(); }
}
// admin ok

$filePath = __DIR__ . '/../uploads/' . (string)$row['arquivo_nome_servidor'];
if (!is_file($filePath)) { echo "<p>Ficheiro não encontrado no servidor.</p>"; exit(); }

$mime     = (string)($row['arquivo_mime'] ?? 'application/octet-stream');
$original = (string)($row['arquivo_nome_original'] ?? 'arquivo');

// Só alguns tipos funcionam bem inline (PDF, imagens e texto)
$inlineOk = (
...
if (!$inlineOk) {
  // Se não for possível visualizar, manda baixar
  header('Location: download_entrega.php?id_entrega=' . $id_entrega);
  exit();
}

// Permite abrir dentro de iframe da própria app (mesmo domínio)
